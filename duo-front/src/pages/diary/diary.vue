<template>
  <view class="diary-container">
    <u-list @scrolltolower="loadMore">
      <u-list-item v-for="item in diaryList" :key="item.id" @click="goDetail(item.id)">
        <view class="diary-item">
          ...
          <view class="diary-info">
            ...
            <view class="diary-tags">
              <u-tag v-for="tag in item.tagList" :key="tag" :text="tag" type="primary" size="mini" />
            </view>
          </view>
        </view>
      </u-list-item>
...
const loadDiaryList = async () => {
  try {
    // 这里替换为实际的API调用
    const res = await uni.$u.http.get('/api/diary/list', {
      params: {
        page: page.value,
